class DropBoxController {
    ...
    connectFirebase() {
        const firebaseConfig = {
            apiKey: "AIzaSyAwO9sayTpXDLEsFKgNPJnpESeBsFT7hRc",
            authDomain: "database-dropbox.firebaseapp.com",
            databaseURL: "https://database-dropbox-default-rtdb.firebaseio.com",
            projectId: "database-dropbox",
            storageBucket: "database-dropbox.appspot.com",
            messagingSenderId: "560495242319",
            appId: "1:560495242319:web:a7987168300f6db3ab0e3a",
            measurementId: "G-F1L6BVYM6F"
    ...
    initEvents() {
        ...
        this.btnRename.addEventListener('click', event => {
            let li = this.getSelectedItems()[0];
            let file = JSON.parse(li.dataset.file);

            let newName = prompt('Renomeie o arquivo:', file.name);
            if (newName) {
                file.name = newName;
        ...
        this.listFilesEl.addEventListener('selectionChange', event => {
            switch (this.getSelectedItems().length) {
                case 0:
